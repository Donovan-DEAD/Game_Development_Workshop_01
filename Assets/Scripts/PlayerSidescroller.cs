...
public class PlayerSidescroller : MonoBehaviour
{
    ...
    {
        // Obtenemos la velocidad actual del Rigidbody
        Vector2 currentVelocity = rb.linearVelocity;

        // Actualizamos la componente X con el movimiento horizontal
        currentVelocity.x = horizontalInput * speed;

        // Si se solicitó un salto, modificamos la componente Y
        if (jumpRequested)
        {
        ...
        // Si se soltó el botón de salto (jump cut), reducimos la velocidad vertical si va hacia arriba
        if (jumpCutRequested)
        {
    ...
    {
        bool gr = Physics2D.OverlapCircle(groundCheck.position, groundRadius, groundLayer);
        return gr;
    }
